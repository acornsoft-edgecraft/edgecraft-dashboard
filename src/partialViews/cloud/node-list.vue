<template>
  <K3Panel :header="`${type} Nodes`"
    ...
    <K3DataTable :value="modelValue">
      <K3Column field="node.node_name"
                header="Node Name"></K3Column>
      <K3Column header="Status">
        <template #body><i class="pi pi-check"></i> </template>
      </K3Column>
      <K3Column field="node.ip_address"
                header="IP Address"></K3Column>
      <K3Column field="baremetal.host_name"
                header="Baremetal Host Name"></K3Column>
      <K3Column field="baremetal.bmc_address"
                header="BMC Address"></K3Column>
      <K3Column :style="{ width: '100px' }">
        <template #body="slotProps">
          <K3Button icon="pi pi-trash"
                    class="p-button-danger p-button-text"
                    @click="removeNode(slotProps.data)"
                    v-if="removable(slotProps.index)"></K3Button>
        </template>
      </K3Column>
    </K3DataTable>
  </K3Panel>
